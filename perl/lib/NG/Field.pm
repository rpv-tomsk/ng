...
=head
   ...
   FORM:CHECKBOX_FIRST - Поменять порядок отображения лейбла и чекбокса. Полезно при размещении чекбоксов в столбцах формы.
...
sub _setValue {
    ...
    if ($field->{TYPE} eq "checkbox") {
        ...
        if (!is_empty($value) && -e $value) {
            ...
            $field->{VALUE} = $value;
            delete $field->{DBVALUE}; #DBVALUE формируется в момент вызова dbValue(), т.к. использует значения прочих полей
        }
        else {
            $field->{VALUE} = "";
            $field->{DBVALUE} = undef;

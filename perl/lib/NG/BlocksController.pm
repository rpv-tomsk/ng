...
sub _getBlockKeys {
    ...
    my $keys = {}; 
    if ($bObj->can("getBlockKeys")) {
        $keys = $bObj->getBlockKeys($block->{ACTION}) or return $cms->error("getBlockKeys() блока ".$block->{CODE}." action ".$block->{ACTION}." не вернул значения");
        return $cms->error("getBlockKeys() блока ".$block->{CODE}." action ".$block->{ACTION}." вернул не хэш") unless ref $keys eq "HASH"; 
    };
    $block->{KEYS} = $keys;
    $block->{KEY}  = $keys->{KEY} if exists $keys->{KEY};
    return $block->{KEYS};
};

sub getBlockContent {
	my $self = shift;
    my $code = shift or die "getBlockContent(): no CODE";
	
    my $cms = $self->cms();
    
    my $block = $self->{_hblocks}->{$code} or return $cms->error("NG::BlocksController->getBlockContent(): block $code not found");
    return $self->_getBlockContent($block);
};

sub _getBlockContent {
    my $self = shift;
    my $block = shift;
...
sub getBlockContentText {
    ...
    my $c = $self->getBlockContent($code);
...
sub requestCacheKeys {
    ...
    my $keys = $cms->getCacheContentKeys(\@allBlocks) or return $cms->error();
...
sub hasValidCacheContent {
    ...
    return 0 unless exists $keys->{KEY};
    return 0 unless $ckeys;
...
sub prepareContent {
    ...
    foreach my $block (@{$self->{_blocks}}) {
        ...
        my $c = $self->getBlockContent($blockCode);
        
		return undef unless defined $c && ref $c;        
        return $c if $c eq 0 || $c->is_error();
        return $cms->error("Block $blockCode return unsupported response type") unless $c->is_output();
        ...
        if ($block->{KEY}) {
            ...
            $a->{CODE} = $block->{CODE};
            $a->{KEYS} = $block->{KEYS};
            $a->{KEYS}->{HEADERS} = $h if $h;
            $a->{DATA} = $c->getOutput();
            push @newContent, $a;
        };
    };

    $cms->storeCacheContent(\@newContent) or return $cms->error();
    
    my $content = $cms->getCacheContent(\@cachedBlocks) or return $cms->error();
    foreach my $code (keys %$content) {
        my $block = $self->{_hblocks}->{$code} or return $cms->error("No block found for block $code in getCacheContent() response");
        $block->{CONTENT} = NG::BlockContent->output($content->{$code}, $block->{CACHEKEYS}->{HEADERS}); #TODO: set <HEAD> content from cache
    };
    return 1;
};
...
sub _getTmplBlockContent {
    ...
    return $self->getBlockContentText($blockCode);
};
...
sub getRegionsContent {
	...
	foreach my $r (keys %{$self->{_regions}}) {
        ...
        foreach my $a (sort { $a->{WEIGHT} <=> $b->{WEIGHT}; } @{$self->{_regions}->{$r}}) {
            $c.= $self->getBlockContentText($a->{BLOCK}->{CODE});
        };
		$rContent->{$r} = $c;
	};
...
sub processBlock {
    ...
    my $c = $self->{_bctrl}->_getBlockContent($block);

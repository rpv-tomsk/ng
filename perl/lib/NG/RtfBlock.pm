...
sub getActiveBlock {
    ...
    if ($CONDITION)  {
        ...
        # Отображаем блок CONTENT если класс занаследован от NG::RtfBlock
        return {BLOCK => "CONTENT", PARAMS=>{subpage=>2}} if ... ;

        # При отображении или перенаправлении на другой модуль/блок можно добавить в
        ...
        # getBlockKeys() для этого блока в этом случае вызываться не будет (что плохо).
    }
}

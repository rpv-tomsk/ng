...
sub _setOutput {
    ...
    if (ref $data) {
        return $self->error("Ссылка на данные не является объектом") unless UNIVERSAL::can($data, 'can');
        return $self->error("Объект данных не содержит метода output") unless UNIVERSAL::can($data, 'output');
    };
